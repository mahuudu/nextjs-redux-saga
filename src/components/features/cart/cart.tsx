...
import { cartAction } from "../../../redux/ducks/cart/cartDuck";
...
const Cart = () => {
  ...
  const increaseQty = (cartItem: ICart) => {
    const newQty = cartItem.quantity ? cartItem.quantity + 1 : 1;
  ...
  const amountWithoutTax = cart?.cartItems?.reduce(
    (acc: any, item: ICart) => acc + (item.quantity || 0) * item.price,
    0
  );
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <main className="md:w-3/4">
                <article className="border border-gray-200 bg-white shadow-sm rounded mb-5 p-3 lg:p-5">
                  {cart?.cartItems?.map((cartItem: ICart) => (
                    <div key={JSON.stringify(cartItem)}>
                        ...
                        <div>
                          <div className="leading-5">
                            <p className="font-semibold not-italic">
                              ${(cartItem.price * parseFloat(cartItem?.quantity?.toString() ?? "0")).toFixed(2)}
                            </p>
                    ...
                    <li className="flex justify-between text-gray-600  mb-1">
                      ...
                      <span className="text-green-500">
                        {cart?.cartItems?.reduce(
                          (acc: any, item: ICart) => acc + item.quantity,
                          0
                        )}
                        (Units)
                      </span>
